...
The .dbi helper is never required, but often beneficial for accessing big databases with high-latency requests. It collects bits of the main database file that are key for navigating it, but typically scattered throughout (even after vacuum). These include interior nodes of SQLite's B-trees, and various metadata tables. Prefetching them in the compact .dbi allows the reader to skip sequences of requests to cherry-pick them from the main file.
